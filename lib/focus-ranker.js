...
function rankFocus(allOpenTasks, todayISO, options = {}) {
  ...
  for (const ht of hot) {
    ...
    for (let i = focus.length - 1; i >= 1; i--) {
      if (!focus[i].is_hot && !focus[i].is_pinned) { evictIdx = i; break; }
    }
    if (evictIdx > 0) focus[evictIdx] = prepFocusItem(ht, evictIdx + 1, todayISO);
  }

  // ─── Pin override: a pinned task that didn't make the cut is forced
  // into slot 2 (preserving the hero). Pinned beats diversity but loses
  // to the hero precedence rules and to the hot-eviction rule above (a
  // truly hot task overrides a manual pin). Only one pin is honored —
  // the highest-scored one — to keep three slots predictable.
  const pinned = ranked.filter((t) => t.pinned === true);
  for (const pt of pinned) {
    if (focus.find((f) => f.id === pt.id)) continue;
    // Only place into slot 2 (or 3 if slot 2 is hot).
    if (focus.length < 3) {
      focus.push(prepFocusItem(pt, focus.length + 1, todayISO));
      continue;
    }
    // Replace the lowest-scored non-hot non-pinned item below the hero.
    let evictIdx = -1;
    for (let i = focus.length - 1; i >= 1; i--) {
      if (!focus[i].is_hot && !focus[i].is_pinned) { evictIdx = i; break; }
    }
    if (evictIdx > 0) focus[evictIdx] = prepFocusItem(pt, evictIdx + 1, todayISO);
    break; // only honor the first pin
  }
