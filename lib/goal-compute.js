...
function maxWeightFromWorkouts(goal, workouts) {
  ...
  for (const w of workouts) {
    ...
    for (const ex of exercises) {
      ...
      // v3.34: defensive Array.isArray. `ex.sets || []` fails when
      // ex.sets is a truthy non-iterable — e.g. a workout entry that
      // stored `"sets": 3` (a count) instead of an array of set objects.
      // `for...of 3` throws "number 3 is not iterable" and the whole
      // goals recompute cron crashed on that one row.
...
// v3.17: latest body composition value (weight, body fat %, lean mass,
// etc.) Driven by body_metrics rows, not workouts. The compute_method
// for goals tracking body composition is 'latest_body_value'; goal.metric
// names which column to read (weight_lb, body_fat_pct, lean_mass_lb,
// skeletal_muscle_pct, bmi).
//
// Returns the most recent populated reading since the goal's anchor.
// `source_workout` is null for body-metric goals; `source_body_metric_id`
// and `source_date` are returned instead so the goal_history row can
// trace back to the weigh-in.
...
function latestBodyValueFromMetrics(goal, bodyMetrics) {
  if (!Array.isArray(bodyMetrics) || !bodyMetrics.length) {
    return { value: null, source_workout: null };
  }
  const column = BODY_METRIC_COLUMNS[goal.metric];
  if (!column) return { value: null, source_workout: null };
  // bodyMetrics is expected sorted by measurement_date DESC (newest first).
  // Find the first row with the requested column populated.
  for (const m of bodyMetrics) {
    ...
    return {
      value: n,
      source_workout: null,
      source_body_metric_id: m.id,
      source_date: m.measurement_date,
    };
  }
  return { value: null, source_workout: null };
}
...
const DRIVERS = {
  ...
  // v3.17: body composition goals — driven by body_metrics, not workouts.
  latest_body_value: latestBodyValueFromMetrics,
};

// Compute methods that read from body_metrics instead of workouts. The
// caller (recomputeOneGoal) uses this to decide which dataset to load.
const BODY_METRIC_COMPUTE_METHODS = new Set(['latest_body_value']);

function computeValueForGoal(goal, workouts, bodyMetrics) {
  ...
  // Body-composition drivers read from body_metrics. Workout drivers read
  // from workouts. Both safely receive an empty array when their source
  // dataset wasn't provided.
  if (BODY_METRIC_COMPUTE_METHODS.has(goal.compute_method)) {
    return driver(goal, bodyMetrics || []);
  }
