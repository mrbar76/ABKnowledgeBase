...
function composeCoachRead(signals) {
  ...
  if (shabbat) {
    return {
      lead: 'Shabbat.',
      body: 'The system is resting too.',
      mute: 'See you Saturday night.',
    };
  }
  ...
  if (
    overdue.hot_count > 0 &&
    today.top_focus &&
    today.top_focus.status === 'waiting_on' &&
    today.top_focus.days_waiting > 0
  ) {
    const title = cleanForUI(today.top_focus.title);
    mute = `Then ${title.toLowerCase()}. ${today.top_focus.days_waiting} days sitting.`;
...
module.exports = { cleanForUI, cleanAllForUI, composeCoachRead, SLUG_MAP };
