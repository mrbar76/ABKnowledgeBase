// lib/recovery.js
//
// Canonical recovery scorer. Single source of truth for /api/recovery/score
// and /api/briefing. Six components, weighted:
//   sleep            30%
//   training_load    25%   (TSB EWMA — TrainingPeaks model)
//   muscle_freshness 20%
//   injury           10%
//   nutrition        10%
//   subjective        5%
//
// `computeRecoveryScore({ date, query })` runs the full pipeline:
// fetches inputs, scores each component, returns the same response shape
// /api/recovery/score has emitted historically. Pure helpers below the
// pipeline are exported for reuse.

'use strict';

// v3.32: shared training-load math. computeRecoveryScore uses
// getEffectiveZones + fillMissingTss so the Recovery panel fills
// null-tss rows on the fly exactly like the /insights/training
// dashboard does — without this the two surfaces saw different
// workout sets and reported different CTL/ATL/TSB off the same data.
const { getEffectiveZones, fillMissingTss } = require('./training-load');
...
function getRegionsForWorkout(w) {
  ...
  if (Array.isArray(exercises) && exercises.length > 0) {
    ...
    for (const ex of exercises) {
      if (ex.completed === false) continue;
...
// v3.30: single source of truth for TSB.
//
// Pre-v3.30 this function computed its own per-workout "load" as
// effort × duration_minutes (1-10 × 1-180), then ran an EWMA on those
// values. Meanwhile routes/insights.js's /training endpoint and
// computeTodayTSB() read the workouts.tss column (real TrainingPeaks
// TSS via HR/LTHR) and ran EWMA on that. Same EWMA shape, completely
// different input units — the recovery panel's "TSB" came out roughly
// 2.5-3× larger than the training dashboard's TSB for the same date,
// and the recovery score's "Training Load" component was tuned for
// the inflated scale.
//
// This rewrite reads workouts.tss directly (the same column the
// dashboard reads) so both surfaces compute TSB from one number and
// can never drift apart again. Score-band thresholds re-anchored to
// the TSS scale, aligned with insights.js's existing fresh / neutral
// / fatigued / very_fatigued cutpoints.
//
// Caller contract unchanged: takes (allWorkouts, targetDate), returns
// { score, detail, tsb, ctl, atl }. allWorkouts must include the
// workouts.tss column (computeRecoveryScore's existing 56-day SELECT
// already pulls *, so tss is there).
function computeTrainingLoadScore(allWorkouts, targetDate) {
  const dailyTss = {};
  for (const w of allWorkouts) {
    const tss = Number(w.tss);
    if (!isFinite(tss) || tss <= 0) continue;
    const d = dateStr(w.workout_date);
    dailyTss[d] = (dailyTss[d] || 0) + tss;
  }

  // EWMA aligned with routes/insights.js's ewma(): v_t = v_{t-1}(1-1/N) + tss/N.
  // The prior continuous-time formulation (exp decay) was ~1% off in
  // steady state — close enough that the EWMA shape isn't the bug,
  // but standardizing on one formula closes that gap too.
  let ctl = 0;
  let atl = 0;
  const start = new Date(targetDate + 'T12:00:00');
  start.setDate(start.getDate() - 56);
  for (let d = new Date(start); d <= new Date(targetDate + 'T12:00:00'); d.setDate(d.getDate() + 1)) {
    const ds = d.toISOString().slice(0, 10);
    const tss = dailyTss[ds] || 0;
    ctl = ctl * (1 - 1 / 42) + tss / 42;
    atl = atl * (1 - 1 / 7)  + tss / 7;
  }
  const tsb = Math.round(ctl - atl);

  // Score bands re-anchored to real TSB. Cross-referenced with
  // routes/insights.js status mapping (fresh > 5, neutral > -10,
  // fatigued > -20, very_fatigued ≤ -20) and tsbAlerts (low < -30,
  // crash < -80). Higher score = more recovered.
  let score;
  if (tsb > 25)      score = 70;    // detraining — slight rest beyond optimal
  else if (tsb >= 5) score = 100;   // fresh
  else if (tsb >= -10) score = 95;  // neutral / optimal
  else if (tsb >= -20) score = 80;  // productive fatigue
  else if (tsb >= -30) score = 65;  // fatigued
  else if (tsb >= -50) score = 45;  // accumulated fatigue
  else if (tsb >= -80) score = 25;  // overreaching
  else                  score = 10; // crash / functional overtraining

  score = clamp(score);

  let tsbLabel;
  if (tsb > 25)        tsbLabel = 'detraining';
  else if (tsb >= 5)   tsbLabel = 'fresh';
  else if (tsb >= -10) tsbLabel = 'optimal';
  else if (tsb >= -20) tsbLabel = 'productive';
  else if (tsb >= -30) tsbLabel = 'fatigued';
  else if (tsb >= -50) tsbLabel = 'accumulated fatigue';
...
function nutritionDayScore(summary) {
  ...
  const cal = Number(summary.total_calories);
  const protein = Number(summary.total_protein_g) || 0;
  let s = 50;
...
function generateRecommendation(muscleStatus, injuries) {
  const fatigued = Object.entries(muscleStatus).filter(([, v]) => v.status === 'fatigued').map(([, v]) => v.label);
  const fresh = Object.entries(muscleStatus).filter(([, v]) => v.status === 'fresh').map(([, v]) => v.label);
...
/**
 * Composes the full recovery score for a single date.
 * Single source of truth for /api/recovery/score and /api/briefing.
 *
 * @param {object} opts
 * @param {string} opts.date           ISO date (YYYY-MM-DD)
 * @param {function} opts.query        DB query fn from db.js
 * @returns {Promise<object>}          Same shape as /api/recovery/score
 */
async function computeRecoveryScore({ date, query }) {
  if (!date) throw new Error('computeRecoveryScore: date required');
  if (typeof query !== 'function') throw new Error('computeRecoveryScore: query fn required');
  ...
  // v3.32: fill null-tss rows on the fly, exactly like the
  // /insights/training dashboard does. Without this the recovery
  // panel skipped any workout with stored tss=null (e.g. rows the
  // backfill flagged/skipped, or freshly-synced rows the cron hasn't
  // computed yet) while the dashboard counted them — producing the
  // two-surfaces-one-metric divergence (recovery TSB +7 vs dashboard
  // TSB -21 off identical data). One zones lookup for the window's
  // anchor date covers the EWMA; computeTSS tolerates a null zones row
  // by falling through to the effort path.
  const zones = await getEffectiveZones(date, query);
  fillMissingTss(allWorkouts, zones);
...
module.exports = {
  ...
  // Helpers exported for the route handler's /trend pipeline + tests.
  computeSleepScore,
  ...
  getRegionsForWorkout,
  effortRecoveryMultiplier,
  clamp,
  dateStr,
  MUSCLE_MAP,
  RECOVERY_HOURS,
