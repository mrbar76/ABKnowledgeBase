...
// Slim a task row for JSON payload — keeps the fields the Today UI uses.
function slimTask(t) {
  return {
    id: t.id, title: t.title, priority: t.priority, status: t.status,
    due_date: t.due_date, context: t.context, waiting_on: t.waiting_on,
    updated_at: t.updated_at
  };
}

// Why a task is in the top 3 — used as the meta line under each focus item.
function reasonFor(t, today) {
  if (t.due_date) {
    const d = daysSince(t.due_date, today);
  ...
  if (t.status === 'waiting_on') {
    const w = daysSince(t.updated_at, today) || 0;
    return `waiting on ${t.waiting_on || 'someone'} for ${w}d`;
  }
...
// ══════════════════════════════════════════════════════════════════
//  generateBriefing — single source of truth, returns both
//  the structured JSON payload and the markdown rendering.
// ══════════════════════════════════════════════════════════════════
...
async function generateBriefing(today) {
  ...
  // ── Structured payload (consumed by Today screen + future surfaces) ──
  const topFocus = top3.map(t => ({
    id: t.id,
    title: t.title,
    priority: t.priority,
    due_date: t.due_date,
  ...
  const payload = {
    ...
    due_today:   { count: dueToday.length,    tasks: dueToday.map(slimTask) },
    in_progress: { count: inProgress.length,  tasks: inProgress.map(slimTask) },
    waiting_on:  { count: waiting.length,     tasks: waiting.map(slimTask) },
    stale:       { count: staleTasks.length,  tasks: staleTasks.map(slimTask) },
    yesterday: {
      completed_count: completedYesterday.length,
      workout: yesterdayWorkout ? {
        title: yesterdayWorkout.title || yesterdayWorkout.workout_type || 'Workout',
        effort: yesterdayWorkout.effort,
        type: yesterdayWorkout.workout_type
      } : null,
      nutrition: { calories: cal, protein_g: protein, meal_count: mealCount }
    },
    recovery: {
      sleep_hours: sleepData?.sleep_hours != null ? Number(sleepData.sleep_hours) : null,
      sleep_quality: sleepData?.sleep_quality != null ? Number(sleepData.sleep_quality) : null,
      score: recoveryScore,
      label: recoveryLabel,
      recommendation: recoveryRecommendation
    },
    streaks: { train: streakTrain, execute: streakExecute, recover: streakRecover },
    today_plan: dailyPlan ? {
      id: dailyPlan.id,
      title: dailyPlan.title,
      goal: dailyPlan.goal,
      status: dailyPlan.status,
      workout_type: dailyPlan.workout_type,
      workout_focus: dailyPlan.workout_focus,
      target_effort: dailyPlan.target_effort,
      target_calories: dailyPlan.target_calories,
      target_protein_g: dailyPlan.target_protein_g,
      coaching_notes: dailyPlan.coaching_notes,
  ...
  // ── Build markdown (output preserved byte-for-byte) ──
  const lines = [];
...
//  Returns markdown by default; ?format=json returns the structured
//  payload (consumed by the Today screen).
// ══════════════════════════════════════════════════════════════════

router.get('/', async (req, res) => {
  try {
    const today = req.query.date || req.getToday();
    const { payload, markdown } = await generateBriefing(today);
