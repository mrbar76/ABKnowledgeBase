...
//   2. User visits /api/health/dropbox-auth, follows the link to authorize,
//      pastes the resulting code, and we mint a refresh token they put back
...
async function moveToProcessed(file) {
  ...
  try {
    // Ensure destination folder exists (create_folder_v2 is idempotent on conflict)
...
// Status: shows env config presence + last sync result
...
// One-time OAuth bootstrap. Two-step:
//   GET  /dropbox-auth         → page with the authorize URL + code submit form
//   POST /dropbox-auth { code } → exchanges code for refresh token, returns it
...
<html><head><title>Dropbox auth</title>
...
input{width:100%;padding:10px;font-size:16px;border:1px solid #ccc;border-radius:6px;margin:8px 0}
button{padding:10px 16px;font-size:16px;background:#0061fe;color:#fff;border:0;border-radius:6px}
.step{margin:24px 0;padding:16px;border-left:3px solid #0061fe;background:#f4f8ff}
#out{margin-top:16px;padding:12px;background:#f4f4f4;border-radius:6px;word-break:break-all;font-family:monospace;font-size:13px}
</style></head><body>
...
async function submitCode() {
  ...
  try {
    const r = await fetch('/api/health/dropbox-auth?api_key=${encodeURIComponent(apiKey)}', {
      method: 'POST',
