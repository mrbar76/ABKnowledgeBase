...
router.get('/', async (req, res) => {
  try {
    ...
    const [knowledge, transcripts, tasks, conversations, workouts, bodyMetrics, meals, coachingSessions, injuries] = await Promise.all([
    ...
    res.json({
      query: term,
      results: { knowledge, transcripts, tasks, conversations, workouts, body_metrics: bodyMetrics, meals, coaching_sessions: coachingSessions, injuries },
      total: knowledge.length + transcripts.length + tasks.length + conversations.length + workouts.length + bodyMetrics.length + meals.length + coachingSessions.length + injuries.length,
    });
  } catch (err) {
...
router.post('/ai', async (req, res) => {
  try {
    ...
    const [knowledge, transcripts, tasks, workouts, bodyMetrics, meals, coachingSessions, injuries] = await Promise.all([
